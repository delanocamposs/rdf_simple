samples_data_2017 = {

#HLT_TriplePhoton_20_20_20_CaloIdLV2 not availabole in eras B/C, opted to not process these

#'DoubleEG_2017B':{'dataset':'/DoubleEG/Run2017B-UL2017_MiniAODv2_NanoAODv9-v1/NANOAOD',
#                    'triggers':['HLT_Diphoton30PV_18PV_R9Id_AND_IsoCaloId_AND_HE_R9Id_PixelVeto_Mass55'],
#                    'isMC':0,
#                    'veto_triggers':[],
#                    'era': '2017',
#                    'jobs':16},
#'DoubleEG_2017C':{'dataset':'/DoubleEG/Run2017C-UL2017_MiniAODv2_NanoAODv9-v1/NANOAOD',
#                    'triggers':['HLT_Diphoton30PV_18PV_R9Id_AND_IsoCaloId_AND_HE_R9Id_PixelVeto_Mass55'],
#                    'isMC':0,
#                    'veto_triggers':[],
#                    'era': '2017',
#                    'jobs':16},
'DoubleEG_2017D':{'dataset':'/DoubleEG/Run2017D-UL2017_MiniAODv2_NanoAODv9-v1/NANOAOD',
                    'triggers':['HLT_TriplePhoton_20_20_20_CaloIdLV2'],
                    'isMC':0,
                    'veto_triggers':[],
                    'era': '2017',
                    'jobs':8},
'DoubleEG_2017E':{'dataset':'/DoubleEG/Run2017E-UL2017_MiniAODv2_NanoAODv9-v1/NANOAOD',
                    'triggers':['HLT_TriplePhoton_20_20_20_CaloIdLV2'],
                    'isMC':0,
                    'veto_triggers':[],
                    'era': '2017',
                    'jobs':16},
'DoubleEG_2017F':{'dataset':'/DoubleEG/Run2017F-UL2017_MiniAODv2_NanoAODv9-v1/NANOAOD',
                    'triggers':['HLT_TriplePhoton_20_20_20_CaloIdLV2'],
                    'isMC':0,
                    'veto_triggers':[],
                    'era': '2017',
                    'jobs':20}
}
